...
var wappalyzer = (function () {
  ...
  Application.prototype = {
    ...
    setDetected: function (pattern, type, value, key, category) {
      this.detected = true
      this.category = category

      // Set confidence level
      this.confidence[type + ' ' + (key ? key + ' ' : '') +
      pattern.regex] = pattern.confidence ? pattern.confidence : 100
      ...
      if (pattern.version) {
        ...
        if (matches) {
          matches.forEach(function (match, i) {
            ...
            var ternary = new RegExp('\\\\' + i + '\\?([^:]+):(.*)$').exec(
              version)

            if (ternary && ternary.length === 3) {
              w.log({match: match, i: i, ternary: ternary})

              version = version.replace(ternary[0],
  ...
  var w = {
    ...
    log: function (message, type) {
      ...
      driver('log',
        {message: '[wappalyzer ' + type + '] ' + message, type: type})
    },
    ...
    analyze: function (hostname, url, data) {
      var
        i, j, app, confidence, type, regexMeta, regexScript, match, content,
        meta, header, version, id,
      ...
      for (app in w.apps) {
        ...
        // Appliction
        apps[app] = w.detected[url] && w.detected[url][app]
          ? w.detected[url][app]
          : new Application(app)
            ...
            case 'meta':
              ...
              while (match = regexMeta.exec(data.html)) {
                for (meta in w.apps[app][type]) {
                  ...
                  if (new RegExp('name=["\']' + meta + '["\']', 'i').test(
                    match)) {
            ...
            case 'headers':
              ...
              for (header in w.apps[app].headers) {
                parse(w.apps[app][type][header]).forEach(function (pattern) {
                  var headerContent = data[type][header.toLowerCase()]
                  if (typeof headerContent === 'object') {
                    headerContent.forEach(item => {
                      if (pattern.regex.test(item)) {
                        apps[app].setDetected(pattern, type, item, header)
                      }
                    })
                  } else if (typeof headerContent === 'string'
                    && pattern.regex.test(data[type][header.toLowerCase()])) {
                    apps[app].setDetected(pattern, type, headerContent, header)
                  }
      ...
      w.log('[ profiler ] Tested ' + profiler.regexCount +
        ' regular expressions in ' +
        ((new Date().getTime() - profiler.startTime) / 1000) + 's')
      w.log('[ profiler ] Slowest pattern took ' +
        (profiler.slowest.duration / 1000) + 's: ' + profiler.slowest.app +
        ' | ' + profiler.slowest.type + ' | ' + profiler.slowest.regex)
          ...
          if (w.apps[app] && w.apps[app].implies) {
            ...
            w.apps[app].implies.forEach(function (implied) {
              ...
              if (!apps.hasOwnProperty(implied.string)) {
                apps[implied.string] = w.detected[url] &&
                w.detected[url][implied.string]
                  ? w.detected[url][implied.string]
                  : new Application(implied.string, true)
              ...
              for (id in confidence) {
                apps[implied.string].confidence[id + ' implied by ' +
                app] = confidence[id] *
                  (implied.confidence ? implied.confidence / 100 : 1)
              }
            })
          }
        }
      ...
      w.log(Object.keys(apps).length + ' apps detected: ' +
        Object.keys(apps).join(', ') + ' on ' + url)
      ...
      for (app in apps) {
        ...
        if (w.detected[url][app].getConfidence() >= 100) {
          // Per hostname
          if (/(www.)?((.+?)\.(([a-z]{2,3}\.)?[a-z]{2,6}))$/.test(hostname) &&
      ...
      if (w.ping.hostnames.hasOwnProperty(hostname)) {
        if (typeof data.html === 'string' && data.html) {
          match = data.html.match(
            /<html[^>]*[: ]lang="([a-z]{2}((-|_)[A-Z]{2})?)"/i)
          ...
          while (match = regexMeta.exec(data.html)) {
            ...
            match = match[0].match(
              /name="(author|copyright|country|description|keywords)"[^>]*content="([^"]+)"/i)
